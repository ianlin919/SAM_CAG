...
criterion_G =torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)     #GeneralizedDiceLoss()#
...
def train_sam(args, net: nn.Module, optimizer, train_loader,
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}', unit='img') as pbar:
        for pack in train_loader:
            ...
            # print(type(pack))  # 输出 pack 的类型
            # print(pack.keys())  # 输出 pack 中的键
...
def validation_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
        ...
        for ind, pack in enumerate(val_loader):
            ...
            while (buoy + evl_ch) <= imgsw.size(-1):
                ...
                with torch.no_grad():
                    ...
                    tot += lossfunc(torch.sigmoid(pred), masks)
                    ...
                    # 在文件夹中保存文件
                    # 初始化精确度（Precision）和召回率（Recall）列表


                    th = 0.9
                    SP = get_specificity(pred, masks, th)
                    SE = get_sensitivity(pred, masks, th)
                    PR = get_precision(pred, masks, th)
                    F1 = get_F1(pred, masks, th)
                    pred = predicted_color_result(imgs, pred, masks,th)
                    ...
                    # print(args.path_helper['sample_path'])
                    save_dir = os.path.join(args.path_helper['sample_path'], 'compare')
                    if not os.path.exists(save_dir):
                        os.makedirs(save_dir)
                    # print(save_dir)
                    # save_image(pred, save_file)
                    make_cag_grid(imgs[0, 0, :, :], masks[0, 0, :, :], pred[0, :, :, :],
                                  {"SE": SE, "PR": PR, "F1": F1}, ix, str(save_dir))
    ...
    print(ix)
    print("TH=",th)
    print("average SE:",v_SE / ix)
    print("average PR:", v_PR / ix)
    print ("average F1:",v_F1 / ix)
