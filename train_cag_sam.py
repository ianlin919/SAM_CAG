...
if args.weights != 0:
    ...
    net.load_state_dict(checkpoint.get('state_dict', checkpoint), strict=False)
...
if args.dataset == 'isic':
    ...
    '''REFUGE data'''
    refuge_train_dataset = REFUGE(args, args.data_path, transform=RandAugment_best_2aug_with_ori_img(1, 20),
...
for epoch in range(settings.EPOCH):
    if epoch and epoch < 5:
        if args.dataset != 'REFUGE':
            ...
            tol, (eiou_cup, eiou_disc, edice_cup, edice_disc) = function.validation_sam(args, nice_test_loader, epoch,
                                                                                        net, writer)
            logger.info(
                f'Total score: {tol}, IOU_CUP: {eiou_cup}, IOU_DISC: {eiou_disc}, DICE_CUP: {edice_cup}, DICE_DISC: {edice_disc} || @ epoch {epoch}.')
